var rule = {
    ...
    timeout:5000,
    // 从顶部导航菜单提取分类，排除前2个(li:gt(1))，只取前20个避免抓到尾部链接
    class_parse:'#menus&&li:gt(1):lt(20);a&&Text;a&&href;.*/(.*)/',
    cate_exclude:'欧美剧|旧版6v|留言|求片|标签',
    play_parse:true,
    limit:6,
    推荐: '*',
    // 网站改版：现在每个影片直接是一个<a>标签包裹<img>+<h2>
    // 用 a:has(img) 选出包含图片的链接（即影片卡片）
    一级: '#content&&a:has(img);h2&&Text;img&&src;;a&&href',
    二级: {
